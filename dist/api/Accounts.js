'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _BankAccounts = require('./BankAccounts');

var _BankAccounts2 = _interopRequireDefault(_BankAccounts);

var _Resource = require('./Resource');

var _Resource2 = _interopRequireDefault(_Resource);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

let Accounts = class Accounts extends _Resource2.default {
  create(data) {
    const headers = this.authenticator.secureHeaders('post', this.uri());
    return this.request('post', { uri: this.uri(), headers: headers, body: JSON.stringify(data) });
  }

  update(accountId, data) {
    const headers = this.authenticator.secureHeaders('post', this.uri(accountId));
    return this.request('post', { uri: this.uri(accountId), headers: headers, body: JSON.stringify(data) });
  }

  bankAccounts(accountId) {
    return new _BankAccounts2.default(this.host, this.authenticator, this.uri(accountId));
  }
};
exports.default = Accounts;